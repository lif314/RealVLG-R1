...
class SAMWrapper:
    ...
    def predict(self, image_path: str, bbox: list):
        rgb_image = np.array(Image.open(image_path).convert("RGB"), dtype=np.uint8)
...
def seg_reward(pred_bbox: list, ground_truth: str, sam_model, sm_calculator) -> float:
    ...
    try:
        ...
        mask_path = gt_json.get("mask_path", "")
        image_path = gt_json.get("image_path", "")

        if not mask_path or not image_path:
            return 0.0

        # SAM predict
        mask_pred, _ = sam_model.predict(image_path, pred_bbox)
        mask_pred = np.array(mask_pred).astype(_TYPE)

        # load GT mask
        gt_np = (np.array(Image.open(mask_path).convert("L")) > 127).astype(_TYPE)
