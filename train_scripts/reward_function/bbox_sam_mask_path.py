...
def seg_reward(pred_bbox: list, ground_truth: str, sam_model: SAMWrapper, sm_calculator: Smeasure) -> float:
    """基于 SAM 预测和 GT mask 计算 S-measure 分数"""
    ...
    try:
        ...
        gt_np = (gt_np > 127).astype(_TYPE)

        # 数据归一化
        mask_pred = (mask_pred - mask_pred.min()) / (mask_pred.max() - mask_pred.min() + _EPS)

        sm_score = sm_calculator.cal_sm(mask_pred, gt_np)
        return max(0.0, min(1.0, sm_score))
    except Exception:
        return 0.0

def bbox_reward(pred_bbox: list, ground_truth: str) -> float:
    """计算预测 bbox 与 GT bbox 的 IoU"""
    try:
        json_data = json.loads(ground_truth)
        if isinstance(json_data, dict):
            gt_box = json_data.get("bbox", [])
        elif isinstance(json_data, list) and len(json_data) == 4:
            gt_box = json_data
        else:
            return 0.0
        xA = max(pred_bbox[0], gt_box[0])
        yA = max(pred_bbox[1], gt_box[1])
        xB = min(pred_bbox[2], gt_box[2])
        yB = min(pred_bbox[3], gt_box[3])
...
def clean_response(response: str) -> str:
    ...
    response = re.sub(r">\s+<", "><", response)  # 标签之间多余空格
    return response
 
def get_pred_bbox(response: str) -> list:
    """提取预测 bbox"""
    ...
    match = COORD_PATTERN.fullmatch(answer_str)
    if not match:
        return None
    try:
...
def format_reward(response: str) -> float:
    ...
    re_str = (
        r"<think>.*?</think>\s*"
        r"<answer>\(\s*[\d\.eE+-]+\s*,\s*[\d\.eE+-]+\s*\)\s*,\s*"
        r"\(\s*[\d\.eE+-]+\s*,\s*[\d\.eE+-]+\s*\)</answer>"
    )
    pattern = re.compile(re_str, re.DOTALL)
    return 1.0 if pattern.search(response) else 0.0

def compute_score(reward_inputs: List[dict], format_weight: float = 0.1, sam_model=None) -> List[dict]:
    """计算 batch 的格式 + bbox + seg reward"""
